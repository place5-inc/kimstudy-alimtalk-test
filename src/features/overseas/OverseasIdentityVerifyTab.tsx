...
export function OverseasIdentityVerifyTab() {
  ...
  const handleSubmit = useCallback(
    async (e: React.FormEvent) => {
      ...
      if (!birth.trim())     { setResult({ ok: false, message: "생년월일을 입력해주세요." }); return; }
      ...
      try {
        ...
        if (r.ok && json.isSuccess) {
          showToast("완료되었습니다");
          setResult({ ok: true, message: json.systemMessage ?? "신원인증 완료처리 성공" });
        } else {
  ...
  return (
    <div>
      <p className="page-title">신원인증 완료처리</p>
      <p className="page-subtitle">해외 사용자의 생년월일·국적을 등록하고 신원인증을 완료 처리합니다.</p>

      <form className="section" onSubmit={(e) => void handleSubmit(e)} noValidate>
        <p className="section-title">신원인증 완료처리</p>
