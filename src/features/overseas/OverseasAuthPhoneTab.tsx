...
export function OverseasAuthPhoneTab() {
        ...
        if (r.ok) {
          try {
            ...
            if (data.isSuccess && data.code) {
              ...
              setResult({ ok: false, message: data.systemMessage ?? "인증번호 조회 실패" });
            }
          } catch {
          ...
          try {
            const data = JSON.parse(r.body) as AuthResponse;
            setResult({ ok: false, message: data.systemMessage ?? `실패 (${r.status})` });
          } catch {
  ...
  return (
    <div>
      <p className="page-title">해외 휴대폰 - 인증번호 확인</p>
      <p className="page-subtitle">
        해외 번호로 발송된 인증번호를 확인할 수 있어요.
      </p>
