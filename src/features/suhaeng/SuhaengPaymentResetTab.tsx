...
export function SuhaengPaymentResetTab() {
  ...
  const handleSubmit = useCallback(
    async (e: React.FormEvent) => {
      ...
      setResult(null);
      try {
        const r = await callProxy("/admin/test/reset/kimsuhaeng/payment/log", {
          nickname: nickname.trim(),
        });

        const json = JSON.parse(r.body) as { isSuccess: boolean; systemMessage: string | null };
        if (r.ok && json.isSuccess) {
          setResult({ ok: true, message: json.systemMessage || "결제이력 초기화 완료" });
        } else {
  ...
  return (
    <div>
      <p className="page-title">(결제)김수행 결제이력 초기화</p>
      <p className="page-subtitle">
        닉네임에 해당하는 유저의 <code>kimsuhaeng_payment_log</code>를 전부 삭제합니다.
        <br />
        테스트 목적으로 추가한 결제이력(<code>"test" prefix</code>)을 정리할 때 사용합니다.
      </p>

      <form className="section" onSubmit={(e) => void handleSubmit(e)} noValidate>
        <p className="section-title">결제이력 초기화</p>

        <div className="field">
          <label htmlFor="spr_nickname">
